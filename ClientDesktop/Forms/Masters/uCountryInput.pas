...
type
  TfCountryInput = class(TForm)
    lytCountryInput: TLayout;
    lytTool: TLayout;
...
var
  fCountryInput: TfCountryInput;

implementation

{$R *.fmx}

uses uClientCmd, cCountry, uClientFrmCursor, uBusamAutoNusantara, uCountry;

var
  oldCode, oldDescription, oldNote: string;

procedure TfCountryInput.CommandPass(DocAction: string);
begin
  ClearAllControls;
...
procedure TfCountryInput.btnCancelClick(Sender: TObject);
begin
  ClearAllControls;
  fMain.pclMain.ActivePage.Text:= Caption;
end;

procedure TfCountryInput.btnDeleteClick(Sender: TObject);
begin
  ...
  try
    ...
    if btnDelete.Text= 'Delete' then begin
      ...
      TDialogService.MessageDialog('Do you want to restore '+ edtCode.Text+ //edtNumberID.Text.Trim+
        ' data?', TMsgDlgType.mtConfirmation,
...
procedure TfCountryInput.SaveDelete(ADelete: boolean);
var
  Response: string;
...
begin
  ...
  try
    ...
    ObjUpdateData.AddPair(TcCountry.pkid, mynumber.Trim);
    ...
    Response:= TcCountry.DeleteCountryByID(ArrUpdateData.ToJSON);//update database with response
  finally
...
procedure TfCountryInput.btnNewClick(Sender: TObject);
begin
  TmClientFrmCursor.CursorOverride:= crHourGlass;
  try
    fMain.CurrentGlobalData;
...
procedure TfCountryInput.btnSaveClick(Sender: TObject);
begin
  TmClientFrmCursor.CursorOverride:= crHourGlass;
  try
    fMain.CurrentGlobalData;
...
function TfCountryInput.ValidatingData: boolean;
var
...
begin
  ...
  if lblAction.Text= 'NEW' then begin
    JSONCodeByCode:= '';
    JSONCodeByCode:= TcCountry.CheckCodeByCode(edtCode.Text);
    ArrCodeByCode:=
    ...
    try
      if ArrCodeByCode.Count> 0 then begin
        ...
        if edtCode.Text= ObjCodeByCode.GetValue<string>('cncode')
          then begin
    ...
    JSONCodeByOtherID:= '';
    JSONCodeByOtherID:= TcCountry.CheckCodeByOtherID(mynumber, edtCode.Text);
    ArrCodeByOtherID:=
    ...
    try
      if ArrCodeByOtherID.Count> 0 then begin
        ...
        if edtCode.Text= ObjCodeByOtherID.GetValue<string>('cncode')
          then begin
          TDialogService.MessageDialog('Code already exist. Please input '+
            'another one.',
            TMsgDlgType.mtWarning, [TMsgDlgBtn.mbOK], TMsgDlgBtn.mbOK, 0, nil);
...
procedure TfCountryInput.SaveNew;
var
  Response: string;
...
begin
  ...
  try
    ...
    ObjNewData.AddPair('cncode', edtCode.Text.Trim);
    ...
    Response:= TcCountry.CreateCountry(ArrNewData.ToJSON, gserverloc);//update database with response
  finally
  ...
  try
    if ObjNewData.GetValue<boolean>('success')= True then begin
      ...
      fMain.pclMain.ActivePage.Text:= 'Edit Country';
      CommandPass('btnEdit');
    end;
  finally
...
procedure TfCountryInput.SaveEdit;
var
  Response: string;
...
begin
  ...
  if Modified= True then begin
    ...
    try
      ...
      ObjEditData.AddPair(TcCountry.pkid, mynumber.Trim);
      ObjEditData.AddPair('edituser', gusername.Trim);
      ObjEditData.AddPair('note', mmoNote.Text);

      ObjEditData.AddPair('cncode', edtCode.Text.Trim);
      ObjEditData.AddPair('description', edtDescription.Text.Trim);
      ArrEditData.AddElement(ObjEditData);
      Response:= TcCountry.UpdateCountryByID(ArrEditData.ToJSON);//update database with response
    finally
...
procedure TfCountryInput.ClearAllControls;
begin
  lblAction.Text:= '';
  lblDelete.Visible:= False;
...
procedure TfCountryInput.LoadData;
var
  JSONCountryID: string;
  ObjCountryID: TJSONObject;
  ArrCountryID: TJSONArray;
begin
  JSONCountryID:= '';
  JSONCountryID:= TcCountry.ReadCountryByID(mynumber);
  ArrCountryID:= TJSONObject.ParseJSONValue(JSONCountryID) as TJSONArray;
  try
    if ArrCountryID.Count> 0 then begin
      ObjCountryID:= ArrCountryID.Items[0] as TJSONObject;
      if ObjCountryID.GetValue<boolean>('inactive')= True then begin
        lblDelete.Visible:= True;
        btnDelete.Text:= 'Restore';
      ...
      lblNewUser.Text:= ObjCountryID.GetValue<string>('newuser').Trim;
      lblNewDate.Text:= DateTimeToStr(TmClientCmd.JSONDateTimeToDateTime(
        ObjCountryID.GetValue<string>('newdate')));
      lblEditUser.Text:= ObjCountryID.GetValue<string>('edituser').Trim;
      lblEditDate.Text:= DateTimeToStr(TmClientCmd.JSONDateTimeToDateTime(
        ObjCountryID.GetValue<string>('editdate')));
      edtNumberID.Text:= TmClientCmd.FormatNumDoc(
        ObjCountryID.GetValue<string>(TcCountry.pkid).Trim);
      mmoNote.Lines.Add(ObjCountryID.GetValue<string>('note'));

      edtCode.Text:= ObjCountryID.GetValue<string>('cncode').Trim;
      edtDescription.Text:= ObjCountryID.GetValue<string>('description').Trim;

      oldNote:= ObjCountryID.GetValue<string>('note');

      oldCode:= ObjCountryID.GetValue<string>('cncode');
      oldDescription:= ObjCountryID.GetValue<string>('description').Trim;
    end;
  finally
    FreeAndNil(ArrCountryID);
  end;
end;

procedure TfCountryInput.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:= TCloseAction.caFree;
end;

procedure TfCountryInput.FormCreate(Sender: TObject);
var
  i: integer;
begin
  Caption:= 'Country';
...
procedure TfCountryInput.FormDestroy(Sender: TObject);
begin
  fCountryInput:= nil;
end;
