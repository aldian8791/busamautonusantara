...
uses
  ...
  FMX.Controls.Presentation, FMX.Memo.Types, VirtualTable, Data.DB, FMX.StdCtrls,
  FMX.ScrollBox, FMX.Controls, FMX.Dialogs, FMX.Graphics, FMX.Types, FMX.Layouts,
  (*DBAware*)Data.Bind.Components, Data.Bind.DBScope(*DBAware*), FMX.TMSFNCTypes,
  FMX.Memo, FMX.TMSFNCCustomControl, FMX.TMSFNCGraphicsTypes, FMX.TMSFNCGraphics,
  FMX.TMSFNCTabSet, FMX.TMSFNCUtils, FMX.TMSFNCPageControl, FMX.TMSFNCButton,
  FMX.DialogService, FMX.wwLookupComboEdit, FMX.wwComboEdit, FMX.wwDataGrid,
  FMX.wwEdit;

type
  TfStateInput = class(TForm)
    lytStateInput: TLayout;
    lytTool: TLayout;
    ...
    procedure lceCountryEnter(Sender: TObject);
  private
    procedure ClearAllControls;
    procedure LoadData;
    procedure LoadDataCountry;
...
var
  fStateInput: TfStateInput;

implementation

{$R *.fmx}

uses uClientCmd, cState, uClientFrmCursor, uBusamAutoNusantara, uState;

var
  vtbCountry: TVirtualtable;
  bdsCountry: TBindSourceDB;
  oldNote, oldCode, oldDescription, oldCountry:string;

procedure TfStateInput.CommandPass(DocAction: string);
begin
  ClearAllControls;
...
procedure TfStateInput.btnCancelClick(Sender: TObject);
begin
  ClearAllControls;
  fMain.pclMain.ActivePage.Text:= Caption;
end;

procedure TfStateInput.SaveDelete(ADelete: boolean);
var
  Response: string;
...
begin
  ...
  try
    ...
    ObjUpdateData.AddPair(TcState.pkid, mynumber.Trim);
    ...
    Response:= TcState.DeleteStateByID(ArrUpdateData.ToJSON);//update database with response
  finally
...
procedure TfStateInput.btnDeleteClick(Sender: TObject);
begin
...
procedure TfStateInput.btnNewClick(Sender: TObject);
begin
  TmClientFrmCursor.CursorOverride:= crHourGlass;
  try
    fMain.CurrentGlobalData;
...
procedure TfStateInput.btnSaveClick(Sender: TObject);
begin
  TmClientFrmCursor.CursorOverride:= crHourGlass;
  try
    fMain.CurrentGlobalData;
...
function TfStateInput.ValidatingData: boolean;
var
...
begin
  ...
  if lceCountry.Text= '' then begin
    TDialogService.MessageDialog('The Country Code is empty. Please select '+
      'Country Code.', TMsgDlgType.mtWarning, [TMsgDlgBtn.mbOK], TMsgDlgBtn.mbOK,
      0, nil);
    lceCountry.SetFocus;
    lceCountry.DropDown;
    Exit(False);
  ...
  if lblAction.Text= 'NEW' then begin
    JSONCodeByCode:= '';
    JSONCodeByCode:=
      TcState.CheckCodeByCode(edtCode.Text);
    ArrCodeByCode:=
    ...
    try
      if ArrCodeByCode.Count> 0 then begin
        ...
        if edtCode.Text= ObjCodeByCode.GetValue<string>('stcode')
          then begin
    ...
    JSONCodeByOtherID:= '';
    JSONCodeByOtherID:=
      TcState.CheckCodeByOtherID(mynumber, edtCode.Text);
    ArrCodeByOtherID:=
    ...
    try
      if ArrCodeByOtherID.Count> 0 then begin
        ...
        if edtCode.Text= ObjCodeByOtherID.GetValue<string>('stcode')
          then begin
          TDialogService.MessageDialog('Code already exist. Please input '+
            'another one.', TMsgDlgType.mtWarning, [TMsgDlgBtn.mbOK],
            TMsgDlgBtn.mbOK, 0, nil);
...
procedure TfStateInput.SaveNew;
var
  Response: string;
...
begin
  ...
  try
    ...
    ObjNewData.AddPair('stcode', edtCode.Text.Trim);
    ObjNewData.AddPair('description', edtDescription.Text.Trim);
    ObjNewData.AddPair('cnid',
      bdsCountry.DataSet.FieldByName('cnid').AsString.Trim);

    ObjNewData.AddPair('location', gserverloc.Trim);
    ArrNewData.AddElement(ObjNewData);
    Response:= TcState.CreateState(ArrNewData.ToJSON, gserverloc);//update database with response
  finally
...
procedure TfStateInput.SaveEdit;
var
  Response: string;
...
begin
  ...
  if lceCountry.Text<> oldCountry then Modified:= True;
  if Modified= True then begin
    ArrEditData:= TJSONArray.Create;
    try
      ObjEditData:= TJSONObject.Create;
      ObjEditData.AddPair(TcState.pkid, mynumber.Trim);
      ObjEditData.AddPair('edituser', gusername.Trim);
      ObjEditData.AddPair('note', mmoNote.Text);

      ObjEditData.AddPair('stcode', edtCode.Text.Trim);
      ObjEditData.AddPair('description', edtDescription.Text);
      ObjEditData.AddPair('cnid',
        bdsCountry.DataSet.FieldByName('cnid').AsString);
      ArrEditData.AddElement(ObjEditData);
      Response:= TcState.UpdateStateByID(ArrEditData.ToJSON);//update database with response
    finally
...
procedure TfStateInput.ClearAllControls;
begin
  lblAction.Text:= '';
  lblDelete.Visible:= False;
  ...
  lceCountry.Enabled:= False;
end;

procedure TfStateInput.LoadData;
var
  JSONStateByID: string;
  ObjStateByID: TJSONObject;
  ArrStateByID: TJSONArray;
begin
  JSONStateByID:= '';
  JSONStateByID:= TcState.ReadStateByID(mynumber);
  ArrStateByID:= TJSONObject.ParseJSONValue(JSONStateByID) as TJSONArray;
  try
    if ArrStateByID.Count> 0 then begin
      ObjStateByID:= ArrStateByID.Items[0] as TJSONObject;
      if ObjStateByID.GetValue<boolean>('inactive')= True then begin
        lblDelete.Visible:= True;
        btnDelete.Text:= 'Restore';
      ...
      lblNewUser.Text:= ObjStateByID.GetValue<string>('newuser').Trim;
      lblNewDate.Text:= DateTimeToStr(TmClientCmd.JSONDateTimeToDateTime(
        ObjStateByID.GetValue<string>('newdate')));
      lblEditUser.Text:= ObjStateByID.GetValue<string>('edituser').Trim;
      lblEditDate.Text:= DateTimeToStr(TmClientCmd.JSONDateTimeToDateTime(
        ObjStateByID.GetValue<string>('editdate')));
      edtNumberID.Text:= TmClientCmd.FormatNumDoc(
        ObjStateByID.GetValue<string>(TcState.pkid).Trim);
      mmoNote.Lines.Add(ObjStateByID.GetValue<string>('note'));

      edtCode.Text:= ObjStateByID.GetValue<string>('stcode').Trim;
      edtDescription.Text:= ObjStateByID.GetValue<string>('description').Trim;
      lceCountry.Text:= ObjStateByID.GetValue<string>('cncode').Trim;

      oldNote:= ObjStateByID.GetValue<string>('note');

      oldCode:= ObjStateByID.GetValue<string>('stcode');
      oldDescription:= ObjStateByID.GetValue<string>('description');
      oldCountry:= ObjStateByID.GetValue<string>('cncode').Trim;
    end;
  finally
    FreeAndNil(ArrStateByID);
  end;
end;

procedure TfStateInput.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:= TCloseAction.caFree;
end;

procedure TfStateInput.FormCreate(Sender: TObject);
var
  i: integer;
begin
  Caption:= 'State';
  ...
  bdsCountry:= TBindSourceDB.Create(Self);
...
procedure TfStateInput.LoadDataCountry;
var
  JSONCountry: string;
begin
  JSONCountry:= TcState.GetCountry;
  vtbCountry.DisableControls;
  try
    with vtbCountry do begin
      Active:= False;
      IndexFieldNames:= '';
      DeleteFields;
      AddField('cncode', ftString, 3, True);
      AddField('description', ftString, 50, True);
      AddField('cnid', ftString, 50, True);
      IndexFieldNames:= 'description ASC';
      CachedUpdates:= False;
      Active:= True;
      TmClientCmd.JSONToVirtualTable(JSONCountry, vtbCountry);
    end;
    with lceCountry do begin
      bdsCountry.DataSet:= vtbCountry;
      LookupSource:= bdsCountry;
      LookupField:= 'cncode';
      DropDownCount:= 6;
    ...
    vtbCountry.EnableControls;
  end;
end;

procedure TfStateInput.FormDestroy(Sender: TObject);
begin
  FreeAndNil(bdsCountry);
  FreeAndNil(vtbCountry);
  fStateInput:= nil;
end;

procedure TfStateInput.lceCountryEnter(Sender: TObject);
begin
  lceCountry.DropDown;
end;
